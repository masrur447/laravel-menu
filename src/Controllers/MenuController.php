...
namespace Bdacademy\LaravelMenu\Controllers;
...
class MenuController extends Controller
{
    ...
    public function addcategorypostmenu()
    {
        // Decode the JSON arrays
        $ids = json_decode(request()->input('ids'));
        $names = json_decode(request()->input('names'));
        $slugs = json_decode(request()->input('slugs'));

        foreach ($ids as $index => $id) {

            $menuitem = new MenuItems();
            $menuitem->label = $names[$index];
            $menuitem->link = $slugs[$index];
            if (config('menu.use_roles')) {
